<x-guest-layout>
    <!-- Session Status -->
    <x-auth-session-status class="mb-4" :status="session('status')" />

    <form method="POST" action="{{ route('login') }}">
        @csrf

        <!-- RUT -->
        <div>
            <x-input-label for="int_rut" :value="__('RUT')" />
            <x-text-input id="int_rut" class="block mt-1 w-full" type="text" name="int_rut" :value="old('int_rut')"
                required autofocus autocomplete="username" />
            <x-input-error :messages="$errors->get('int_rut')" class="mt-2" />
        </div>

        <!-- Password -->
        <div class="mt-4">
            <x-input-label for="password" :value="__('Password')" />

            <x-text-input id="password" class="block mt-1 w-full" type="password" name="password" required
                autocomplete="current-password" />

            <x-input-error :messages="$errors->get('password')" class="mt-2" />
        </div>

        <!-- Remember Me -->
        <div class="block mt-4">
            <label for="remember_me" class="inline-flex items-center group">
                <input id="remember_me" type="checkbox"
                    class="rounded border-gray-700 bg-[#0a0a0a] text-brand-red shadow-sm focus:ring-brand-red"
                    name="remember">
                <span
                    class="ms-2 text-sm text-gray-400 group-hover:text-gray-200 transition-colors">{{ __('Remember me') }}</span>
            </label>
        </div>

        <div class="flex items-center justify-end mt-4">

            <x-primary-button class="ms-3">
                {{ __('Log in') }}
            </x-primary-button>
        </div>
    </form>
</x-guest-layout>